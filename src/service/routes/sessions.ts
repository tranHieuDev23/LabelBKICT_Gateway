...
import { ImageListManagementOperator, IMAGE_LIST_MANAGEMENT_OPERATOR_TOKEN } from "../../module/images";
...
export function getSessionsRouter(
    ...
    router.post(
        ...
        asyncHandler(async (req, res) => {
            ...
            const { user, userRoleList, userPermissionList, token } = await sessionManagementOperator.loginWithPassword(
                username,
                password
            );
            res.cookie(LABEL_BKICT_AUTH_COOKIE_NAME, token, getCookieOptions()).json({
                user: user,
    ...
    router.get(
        ...
        asyncHandler(async (_, res) => {
            ...
            res.json({
                ...
                user_permission_list: authenticatedUserInformation.userPermissionList,
            });
        })
    );
    ...
    router.get(
        ...
        asyncHandler(async (req, res) => {
            ...
            const userList = await imageListManagementOperator.searchUserManageableImageUserList(
                authenticatedUserInformation,
                query,
                limit
            );
    ...
    router.get(
        ...
        asyncHandler(async (req, res) => {
            ...
            const query = `${req.query.query}`;
            const limit = +(req.query.limit || DEFAULT_GET_USER_LIST_LIMIT);
            const userList = await imageListManagementOperator.searchUserVerifiableImageUserList(
                authenticatedUserInformation,
                query,
                limit
            );
    ...
    router.get(
        ...
        asyncHandler(async (req, res) => {
            ...
            const query = `${req.query.query}`;
            const limit = +(req.query.limit || DEFAULT_GET_USER_LIST_LIMIT);
            const userList = await imageListManagementOperator.searchUserExportableImageUserList(
                authenticatedUserInformation,
                query,
                limit
            );
