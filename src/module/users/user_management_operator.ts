...
import { IMAGE_SERVICE_DM_TOKEN, USER_SERVICE_DM_TOKEN } from "../../dataaccess/grpc";
import { ImageServiceClient } from "../../proto/gen/ImageService";
import { _UserListSortOrder_Values } from "../../proto/gen/UserListSortOrder";
import { UserServiceClient } from "../../proto/gen/UserService";
import { ErrorWithHTTPCode, getHttpCodeFromGRPCStatus, LOGGER_TOKEN } from "../../utils";
...
export interface UserManagementOperator {
    ...
    addUserCanManageUserImage(userId: number, imageOfUserId: number, canEdit: boolean): Promise<UserCanManageUserImage>;
    ...
    addUserCanVerifyUserImage(userId: number, imageOfUserId: number): Promise<UserCanVerifyUserImage>;
...
export class UserManagementOperatorImpl implements UserManagementOperator {
    ...
    public async getUserList(
        ...
        const filterOptionsProto = this.filterOptionsToFilterOptionsProto.convertUserFilterOptions(filterOptions);
        const sortOrderEnumValue = this.getSortOrderEnumValue(sortOrder);
        const { error: getUserListError, response: getUserListResponse } = await promisifyGRPCCall(
        ...
        const userList: User[] = getUserListResponse?.userList?.map((userProto) => User.fromProto(userProto)) || [];
        ...
        getUserRoleListOfUserListResponse?.userRoleListOfUserList?.forEach((userRoleListOfUser, index) => {
            userRoleList[index] =
                userRoleListOfUser.userRoleList?.map((userRoleProto) => UserRole.fromProto(userRoleProto)) || [];
        });

        const { error: getUserTagListOfUserListError, response: getUserTagListOfUserListResponse } =
            await promisifyGRPCCall(this.userServiceDM.getUserTagListOfUserList.bind(this.userServiceDM), {
                userIdList: userIdList,
            });
        if (getUserTagListOfUserListError !== null) {
            this.logger.error("failed to call user_service.getUserTagListOfUserList()", {
        ...
        getUserTagListOfUserListResponse?.userTagListOfUserList?.forEach((userTagListOfUser, index) => {
            userTagList[index] =
                userTagListOfUser.userTagList?.map((userTagProto) => UserTag.fromProto(userTagProto)) || [];
        });
    ...
    private getSortOrderEnumValue(sortOrder: number): _UserListSortOrder_Values {
        switch (sortOrder) {
            case 0:
            ...
            default:
                throw new ErrorWithHTTPCode("invalid sort order", httpStatus.BAD_REQUEST);
        }
    }
    ...
    public async searchUserList(query: string, limit: number): Promise<User[]> {
        ...
        if (searchUserError !== null) {
            ...
            throw new ErrorWithHTTPCode("Failed to search user list", getHttpCodeFromGRPCStatus(searchUserError.code));
        }
    ...
    public async addUserCanManageUserImage(
        ...
        if (user === null) {
            ...
            throw new ErrorWithHTTPCode("Failed to add user can manage user image", httpStatus.NOT_FOUND);
        }
        ...
        const { error: createUserCanManageUserImageError } = await promisifyGRPCCall(
            this.imageServiceDM.createUserCanManageUserImage.bind(this.imageServiceDM),
            { userId, imageOfUserId, canEdit }
        );
        if (createUserCanManageUserImageError !== null) {
            this.logger.error("failed to call image_service.createUserCanManageUserImage()", {
                error: createUserCanManageUserImageError,
            });
            throw new ErrorWithHTTPCode(
                "Failed to add user can manage user image",
                getHttpCodeFromGRPCStatus(createUserCanManageUserImageError.code)
            );
        }
    ...
    public async getUserCanManageUserImageListOfUser(
        ...
        if (user === null) {
            this.logger.error("user cannot be found", { userId });
            throw new ErrorWithHTTPCode("Failed to get user can manage user image list", httpStatus.NOT_FOUND);
        }
        ...
        const totalUserCount = getUserCanManageUserImageOfUserIdResponse?.totalUserCount || 0;
        const userCanManageUserImageProtoList = getUserCanManageUserImageOfUserIdResponse?.userList || [];
        const userCanManageUserImageList = await Promise.all(
            userCanManageUserImageProtoList.map((proto) =>
                this.userCanManageUserImageProtoToUserCanManageUserImageConverter.convert(proto)
            )
        );
    ...
    public async updateUserCanManageUserImage(
        ...
        if (user === null) {
            this.logger.error("user cannot be found", { userId });
            throw new ErrorWithHTTPCode("Failed to update user can manage user image", httpStatus.NOT_FOUND);
        }
        const imageOfUser = await this.userInfoProvider.getUser(imageOfUserId);
        if (imageOfUser === null) {
            this.logger.error("user cannot be found", { imageOfUserId });
        ...
        const { error: updateUserCanManageUserImageError } = await promisifyGRPCCall(
            this.imageServiceDM.updateUserCanManageUserImage.bind(this.imageServiceDM),
            { userId, imageOfUserId, canEdit }
        );
        if (updateUserCanManageUserImageError !== null) {
            this.logger.error("failed to call image_service.updateUserCanManageUserImage()", {
                error: updateUserCanManageUserImageError,
            });
            throw new ErrorWithHTTPCode(
                "Failed to update user can manage user image",
                getHttpCodeFromGRPCStatus(updateUserCanManageUserImageError.code)
            );
        }
        return new UserCanManageUserImage(User.fromProto(imageOfUser), canEdit);
    }
    ...
    public async deleteUserCanManageUserImage(userId: number, imageOfUserId: number): Promise<void> {
        ...
        if (user === null) {
            this.logger.error("user cannot be found", { userId });
            throw new ErrorWithHTTPCode("Failed to delete user can manage user image", httpStatus.NOT_FOUND);
        }
        const { error: deleteUserCanManageUserImageError } = await promisifyGRPCCall(
            this.imageServiceDM.deleteUserCanManageUserImage.bind(this.imageServiceDM),
            { userId, imageOfUserId }
        );
        if (deleteUserCanManageUserImageError !== null) {
            this.logger.error("failed to call image_service.deleteUserCanManageUserImage()", {
                error: deleteUserCanManageUserImageError,
            });
            throw new ErrorWithHTTPCode(
                "Failed to delete user can manage user image",
                getHttpCodeFromGRPCStatus(deleteUserCanManageUserImageError.code)
            );
        }
    }

    public async addUserCanVerifyUserImage(userId: number, imageOfUserId: number): Promise<UserCanVerifyUserImage> {
        const user = await this.userInfoProvider.getUser(userId);
        if (user === null) {
            this.logger.error("user cannot be found", { userId });
            throw new ErrorWithHTTPCode("Failed to add user can verify user image", httpStatus.NOT_FOUND);
        }
        const imageOfUser = await this.userInfoProvider.getUser(imageOfUserId);
        if (imageOfUser === null) {
            this.logger.error("user cannot be found", { imageOfUserId });
        ...
        const { error: createUserCanVerifyUserImageError } = await promisifyGRPCCall(
            this.imageServiceDM.createUserCanVerifyUserImage.bind(this.imageServiceDM),
            { userId, imageOfUserId }
        );
        if (createUserCanVerifyUserImageError !== null) {
            this.logger.error("failed to call image_service.createUserCanVerifyUserImage()", {
                error: createUserCanVerifyUserImageError,
            });
            throw new ErrorWithHTTPCode(
                "Failed to add user can verify user image",
                getHttpCodeFromGRPCStatus(createUserCanVerifyUserImageError.code)
            );
        }
    ...
    public async getUserCanVerifyUserImageListOfUser(
        ...
        if (user === null) {
            this.logger.error("user cannot be found", { userId });
            throw new ErrorWithHTTPCode("Failed to get user can verify user image list", httpStatus.NOT_FOUND);
        }
        ...
        const totalUserCount = getUserCanVerifyUserImageOfUserIdResponse?.totalUserCount || 0;
        const userCanVerifyUserImageProtoList = getUserCanVerifyUserImageOfUserIdResponse?.userList || [];
        const userCanVerifyUserImageList = await Promise.all(
            userCanVerifyUserImageProtoList.map((proto) =>
                this.userCanVerifyUserImageProtoToUserCanVerifyUserImageConverter.convert(proto)
            )
        );
    ...
    public async deleteUserCanVerifyUserImage(userId: number, imageOfUserId: number): Promise<void> {
        ...
        if (user === null) {
            this.logger.error("user cannot be found", { userId });
            throw new ErrorWithHTTPCode("Failed to delete user can verify user image", httpStatus.NOT_FOUND);
        }
        const { error: deleteUserCanVerifyUserImageError } = await promisifyGRPCCall(
            this.imageServiceDM.deleteUserCanVerifyUserImage.bind(this.imageServiceDM),
            { userId, imageOfUserId }
        );
        if (deleteUserCanVerifyUserImageError !== null) {
            this.logger.error("failed to call image_service.deleteUserCanVerifyUserImage()", {
                error: deleteUserCanVerifyUserImageError,
            });
            throw new ErrorWithHTTPCode(
                "Failed to delete user can verify user image",
                getHttpCodeFromGRPCStatus(deleteUserCanVerifyUserImageError.code)
            );
        }
    }
}
